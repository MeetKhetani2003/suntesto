        ...
        <section className="w-full py-16 px-6 bg-warm-white">
          <div className="max-w-[1280px] mx-auto">
            {loading ? (
              ...
              Array.from(new Set(sortedProducts.map((p) => (p.category || "").toUpperCase()).filter(Boolean)))
                .filter((category) => selectedCategory === "All" || selectedCategory.toUpperCase() === category)
                .map((category) => {
                  const categoryProducts = sortedProducts.filter((p) => (p.category || "").toUpperCase() === category);
